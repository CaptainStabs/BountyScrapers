...
webpage = "https://fastfoodnutrition.org/jamba-juice"
identifier = "NATIONAL"
            ...
            for row in rows:
                ...
                if not failed:
                    if rn == "Calories":  # Needs to be exact match
                        nutrition_facts["calories"] = float(rd.strip())
                        ...
                        if "mg" in rd:
                            ...
                            rd_converted = float(rd) / 1000
                            nutrition_facts["fat_g"] = [rd_converted]

                            print(nutrition_facts)
                        else:
                            nutrition_facts["fat_g"] = [float(rd.strip('g'))]
                        ...
                        if "mg" in rd:
                            ...
                            rd_int = rd.replace("mg", "")
                            rd_converted = float(rd) / 1000
                            nutrition_facts["carbohydrates_g"] = [rd_converted]

                        else:
                            nutrition_facts["carbohydrates_g"] = [float(rd.strip('g'))]
                        ...
                        if "mg" in rd:
                            ...
                            nutrition_facts["protein_g"] = [float(rd.strip('g'))]
                        # print(nutrition_facts)

                    elif "Sodium" in rn:
                        if "mg" in rd:
                            nutrition_facts["sodium_mg"] = [float(rd.strip('mg'))]
                        elif "g" in rd:
                            print("G ALERT")
                            print(food_name, rn, rd)
                            rd_int = rd.strip("g")
                            rd_converted = float(rd_int) * 1000
                            nutrition_facts["sodium_mg"] = [rd_converted]

                    elif "Cholesterol" in rn:
                        if "mg" in rd:
                            nutrition_facts["cholesterol_mg"] = [float(rd.strip('mg'))]
                        elif "g" in rd:
                            print("G ALERT")
                            print(food_name, rn, rd)
                            rd_int = rd.strip("g")
                            rd_converted = float(rd_int) * 1000
                            nutrition_facts["cholesterol_mg"] = [rd_converted]
                        ...
                        if "mg" in rd:
                            ...
                            rd_int = rd.replace("mg", "")
                            rd_converted = float(rd) / 1000
                            nutrition_facts["fiber_g"] = [rd_converted]
                        elif "g" in rd:
                            nutrition_facts["fiber_g"] = [float(rd.strip('g'))]
                        ...
                        if "mg" in rd:
                            ...
                            rd_int = rd.replace("mg", "")
                            rd_converted = float(rd) / 1000
                            nutrition_facts["sugars_g"] = [rd_converted]
                        else:
                            nutrition_facts["sugars_g"] = [float(rd.strip('g'))]
