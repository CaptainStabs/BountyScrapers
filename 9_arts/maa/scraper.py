...
with open(filename, "a", encoding='utf-8', newline='') as output_file:
    ...
    if os.path.exists(filename) and os.stat(filename).st_size > 299:
        next = get_last_id(filename, 299, "source_2", url=True)
        # start_id = 3320
    else:
                ...
                if jdr:
                    for jd in jdr["results"]:
                        data = {
                            ...
                            "culture": "|".join([x["name"] for x in jd["culture_group"]]),
                            ...
                            "department": jd.get("department", {}).get("name", None) if jd["department"] else None,
