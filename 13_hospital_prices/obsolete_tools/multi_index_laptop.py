...
def work(url, out):
    try:
        ...
        while tries < 2:
            try:
                index_file_to_csv(url=str(url), out_dir=out)
                tries = 6
            except EOFError:
                print(f"\n!!!EOFError: {url}")
                print(f"\nRetrying, try: {tries}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.make_pr:
            out = os.path.join("C:\\Users\\adria\\github\\BountyScrapers\\13_quest_tic\\", out)
            os.system(f'cd C:\\Users\\adria\\_Bounty\\hospital-prices-allpayers\\ & import_folders_push.bat {out} "{args.commit_message}" {args.branch} {args.processes}')
