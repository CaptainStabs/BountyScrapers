...
import logging
logging.basicConfig()
log = logging.getLogger('mrfutils.flatteners')
log.setLevel(logging.WARNING)


#  python cli_multi.py -mp True -he 300 -o UHC_4 -cm "300 UHC files"
...
def work(url, out):
    try:
        ...
        #         print(f"\nRetrying, try: {tries}")
        #         tries += 1
        url = "https://developers.humana.com/Resource/DownloadPCTFile?fileType=innetwork&fileName=" + url.replace("?fileType=innetwork&fileName=", "")
        # url = "https://storage.googleapis.com/cms-humana-price-transparency-prd/prod/february/inn/" + url.strip("\n")
        try:
            in_network_file_to_csv(url=url, out_dir=out, code_filter=import_csv_to_set(".\\codes\\70_shoppables.csv"), npi_filter=import_csv_to_set(".\\codes\\npis.csv"))
        except EOFError:
            print(f"\n!!!EOFError: {url}")
        except Exception as e:
            tb.print_exc()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', default="F:\\_Bounty\\humana_payers.csv")
