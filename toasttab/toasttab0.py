...
webpage = "https://www.toasttab.com/davoscalzones/v3"
    ...
    for i in range(len(searched)):  # Loop over array (top-level)
        ...
        for j in range(len(menu)):
            ...
            for item_list in category_items:
                ...
                try:
                    item_name = item_list[0].replace('\\"', " inch ").replace('"', " inch ").replace("�", "'").upper()
                    if not first_loop:
                        df = pd.DataFrame.from_dict(nutrition_facts)
                        exists_name = item_name in df.name
                        if exists_name:
                            print(df.loc[df['name'].isin()])

                    nutrition_facts["name"] =  item_name
