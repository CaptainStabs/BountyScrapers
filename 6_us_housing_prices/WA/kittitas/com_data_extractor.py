...
# "ParcelNumber","SaleDate","OriginalSalePrice","Grantor","Grantee","DepartmentOfRevenueCode","Situs","StateCode","Style","YearBuilt"
...
with open(".\\input_data\\Commercial.csv", "r") as input_csv:
    line_count = len([line for line in input_csv.readlines()])
    input_csv.seek(0)
    reader = csv.DictReader(input_csv)

    with open(".\\extracted_data\\com_extracted_data.csv", "a", newline="") as output_csv:
            ...
            try:
                try:
                    if row["OriginalSalePrice"]:
                        ...
                        type_list = [str(row["StateCode"]).strip(), str(row["Style"]).strip()]
