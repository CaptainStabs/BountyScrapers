...
columns = ["property_id", "county", "city", "physical_address", "property_type", "book", "page", "sale_date", "year_built", "sale_price", "zip5", "state", "source_url"]
        ...
        for row in tqdm(reader, total=line_count):
            try:
                land_info = {
                    ...
                    "physical_address": " ".join(str(row["PROP_LOC"]).upper().split()),
                    "property_type": " ".join(str(row["FAC_NAME"]).upper().split()),
                    "sale_date": str(parser.parse(row["DEED_DATE"])),
                ...
                try:
                    if int(row["YR_CONSTR"]) != 0 and int(row["YR_CONSTR"]) <= 2022 and int(row["YR_CONSTR"]) >= 1690:
                        land_info["year_built"] = row["YR_CONSTR"]
