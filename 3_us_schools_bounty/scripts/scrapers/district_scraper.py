...
with GracefulInterruptHandler() as h:
    ...
    with open("districts_testing.csv", "a", encoding="utf-8") as output_source:
        writer = csv.DictWriter(output_source, fieldnames=columns)
        if os.stat("districts_testing.csv").st_size == 0:
            writer.writeheader()
        ...
        for index, row in tqdm(df.iterrows()):
            ...
            if no_error:
                try:
                    search_query = (
                        f"https://nces.ed.gov/ccd/districtsearch/district_list.asp?Search=1&details=1&InstName=&DistrictID={district_id}"
                    )
                ...
                for link in soup.findAll("a"):
                    ...
                    school_info["DISTRICTID"] = row["DISTRICTID"]
