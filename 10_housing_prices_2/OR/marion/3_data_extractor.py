...
columns = ["property_id", "sale_price", "sale_datetime", "property_street_address", "city", "state", "zip5", "seller_name", "buyer_name", "building_year_built", "sale_type", "source_url", "land_area_acres", "land_area_sqft", "buyer_1_state", "seller_1_state"]

with open(f"{folder}_extracted_data.csv", "a", encoding="utf-8", newline="") as output_csv:
        ...
        with open(os.path.join(directory,file), "r", encoding="utf-8") as input_csv:
            ...
            for row in reader:
                ...
                if "Missing Situs Address Record" not in row["SITUSSTREET"]:
                    ...
                    land_info["property_street_address"] = ' '.join(str(' '.join(filter(None, street_list)).upper()).split())
                    ...
                    if sale_date != "":
                        land_info["sale_datetime"] = parser.parse(sale_date)
                    ...
                    if land_info["sale_price"] and land_info["sale_datetime"] and land_info["property_street_address"]:
