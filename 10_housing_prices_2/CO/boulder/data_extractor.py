import csv
from tqdm import tqdm
from dateutil import parser
from pathlib import Path
import sys

from pathlib import Path
import sys

p = Path(__file__).resolve().parents[2]
sys.path.insert(1, str(p))
from _sale_type.sale_type import sale_type
from _common import date_parse



# "strap","Tdate","deed_type","price",,"city","bldgClassDscr","builtYear","UnitCount"
columns = ["property_id", "sale_datetime", "sale_type", "sale_price", "property_street_address", "property_city", "property_type", "year_built", "unit_count",  "property_zip5", "property_county", "state", "source_url", "building_area_sqft", "building_num_beds", "building_num_baths", "building_num_stories", "building_year_built"]
with open("Parcels.csv", "r") as input_csv:
    line_count = len([line for line in input_csv.readlines()])
    input_csv.seek(0)
    reader = csv.DictReader(input_csv)

    with open("extracted_data.csv", "a", newline="") as output_csv:
        writer = csv.DictWriter(output_csv, fieldnames=columns)
        writer.writeheader()

        for row in tqdm(reader, total=line_count):
            try:
                land_info = {
                    "property_id": str(row["strap"]).strip(),
                    "sale_datetime": str(date_parse(row["Tdate"])),
                    "sale_price": row["price"],
                    "property_city": " ".join(str(row["city"]).upper().split()),
                    "property_type": " ".join(str(row["bldgClassDscr"]).upper().split()),
                    "property_county": "BOULDER",
                    "state": "CO",
                    "source_url": "https://hub.arcgis.com/documents/d39100d04be34354b97ad52874f5e7d4/explore",
                    "building_area_sqft": sum([float(row["bsmtSF"]), float(row["carStorageSF"]), float(row["TotalFinishedSF"])]),
                    "building_num_beds": row["nbrBedRoom"] if float(row["nbrBedRoom"]) else None,
                    "building_num_baths": sum([float(row["nbrThreeQtrBaths"])*0.75, float(row["nbrFullBaths"]), float(row["nbrHalfBaths"])*0.5]),
                    "building_num_stories": row["Stories"]

                }

                if not land_info["building_num_baths"]:
                    land_info["building_num_baths"] = None

                # "mailingAddr1","mailingCity","mailingState","mailingZip",
                # If address is in separate fields
                # "str_num","str_pfx","str","str_sfx","str_unit"
                street_list = [str(row["str_num"]).strip(), str(row["str_pfx"]).strip(), str(row["Str"]).strip(), str(row["str_sfx"]).strip(), str(row["str_unit"]).strip()]

                # concat the street parts filtering out blank parts
                land_info["property_street_address"] = ' '.join(filter(None, street_list)).upper()

                if " ".join(str(row["mailingAddr1"]).upper().split()) == land_info["property_street_address"]:
                    land_info["property_zip5"] = row["mailingZip"][:5]

                # Delete if no year_built
                try:
                    if int(row["builtYear"]) != 0 and int(row["builtYear"]) <= 2022:
                        land_info["building_year_built"] = row["builtYear"]

                except ValueError:
                    pass

                try:
                    # Delete if no zip5
                    if land_info["property_zip5"] == "00000" or land_info["property_zip5"] == "0" or len(land_info["property_zip5"]) != 5:
                        land_info["property_zip5"] = ""
                except KeyError:
                    pass

                try:
                    # Delete if no unit count
                    if int(row["UnitCount"]) != 0:
                        land_info["building_num_units"] = row["UnitCount"]
                except ValueError:
                    pass

                year = land_info["sale_datetime"].split("-")[0]

                if land_info["property_street_address"] and land_info["sale_datetime"] and land_info["sale_price"] != "" and int(year) <= 2022:
                    writer.writerow(land_info)

            except parser._parser.ParserError:
                pass
