...
columns = ["property_id", "property_street_address", "sale_type", "seller_name", "buyer_name", "sale_datetime", "sale_price", "property_county", "state", "source_url", "land_appraised_value", "land_assessed_value", "building_appraised_value", "building_assessed_value", "land_area_sqft", "land_area_acres", "building_area_sqft", "transfer_deed_type"]
    ...
    with open("extracted_data.csv", "a", newline="") as output_csv:
        ...
        for row in tqdm(reader, total=line_count):
            ...
            if row["DeedType"].strip():
                try:
                    land_info["transfer_deed_type"] = str(deed_type[row["DeedType"]]).upper()
                except KeyError:
                    # print(row["DeedType"])
                    land_info["transfer_deed_type"] = row["DeedType"].upper().strip()
            else:
                try:
                    land_info["transfer_deed_type"] = str(deed_type[str(row["SaleVerificationCode"]).upper()]).upper()
                except KeyError:
                    land_info["transfer_deed_type"] = str(row["SaleVerificationCode"]).upper()
             # ,,,
