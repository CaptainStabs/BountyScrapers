	...
	async def __aenter__(self):
		...
		if (self.is_remote and self.suffix == '.json.gz'):
			async with aiohttp.ClientSession(auto_decompress=True) as session:
				async with session.get(self.loc) as self.r:
					# r_content = self.r.content.readexactly(100)
					self.f = gzip.GzipFile(fileobj= self.r.content)
					
		
		elif (self.is_remote and self.suffix == '.json'):
			async with aiohttp.ClientSession(auto_decompress=True) as session:
				async with session.get(self.loc) as self.r:
					# self.r.content.decode_content = True
					self.f = self.r.content.readexactly(100)

		elif self.suffix == '.json.gz':
			self.f = await aiofiles.open(self.loc, 'rb')
