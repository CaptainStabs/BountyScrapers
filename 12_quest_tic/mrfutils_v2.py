...
# tqdm lock
	if lock:
		with lock:
			bar = tqdm(desc=f"{pos}: Provider Reference Map", position=int(pos), leave=True)

	for prefix, event, value in tqdm(parser, desc=f"{pos}: Provider Reference Map", position=int(pos), disable=disable):
...
def _in_network_items(
	...
	if lock:
		with lock:
			bar = tqdm(desc=f"{pos}: In Network Items gen", position=int(pos), leave=True)

	for prefix, event, value in tqdm(parser, desc=f"{pos}: In Network Items gen", position=int(pos), disable=disable):
...
class MRFContent:
	...
	def __init__(self, loc, npi_filter = None, code_filter = None, pos = 0, lock=None):
		self.loc = loc
	...
	def _extractor(self) -> Generator[dict, None, None]:
		...
		with JSONOpen(self.loc) as f:
			...
			if prefix == 'provider_references':
				...
				unprocessed_items = _in_network_items(parser, self.code_filter, self.pos, self.lock)
...
def json_mrf_to_csv(
	...
	content = MRFContent(loc, npi_filter, code_filter, pos, lock)
	content.start()
