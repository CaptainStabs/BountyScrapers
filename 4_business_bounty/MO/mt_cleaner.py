...
input_columns = ["name", "business_type", "state_registered", "street_registered", "city_registered", "zip5_registered", "state_physical", "street_physical", "city_physical", "zip5_physical", "filing_number"]
...
filename = "mt_cleaned_merged_data.csv"

with open("mt_merged_data.csv", "r", encoding="utf-8") as input_csv:
    ...
    with open(filename, "a", newline="", encoding="utf-8") as output_csv:
        ...
        for i, row in tqdm(enumerate(reader), total=total):
            ...
            business_info = {
                ...
                "street_registered": str(row["street_registered"]).strip(","),
                "city_registered": row["city_registered"],
                "street_physical": str(row["street_physical"]).strip(","),
