...
def scraper(lists):
    file_name = "rhode_island.csv"
    columns = ["name", "business_type", "state_registered", "street_registered", "city_registered", "zip5_registered", "state_physical", "street_physical","city_physical","zip5_physical", "filing_number", "naics_2017", "corp_id"]
    last_id = lists[0]
    end_id = lists[1]
    # file_name = lists[2]
    ...
    with open(file_name, "a", encoding="utf8") as output_file:
        ...
        for id_number in tqdm(range(last_id, end_id)):
            ...
            if do_save:
                ...
                if business_type_string:
                    ...
                    try:
                        business_info["state_registered"] = str(parser.xpath('//*[@id="MainContent_lblResidentState"]/text()')[0]).strip().rstrip(",").upper()
                    except IndexError:
                        print("      [!] IndexError on state_registered!")

                    try:
                        business_info["street_registered"] = str(parser.xpath('//*[@id="MainContent_lblResidentStreet"]/text()')[0]).strip().rstrip(",").upper()
                        business_info["city_registered"] = str(parser.xpath('//*[@id="MainContent_lblResidentCity"]/text()')[0]).strip().rstrip(",").upper()
                        business_info["zip5_registered"] = str(parser.xpath('//*[@id="MainContent_lblResidentZip"]/text()')[0]).strip().rstrip(",").upper()
                    except IndexError:
                        print("      [!] IndexError on one of the other registereds!")
