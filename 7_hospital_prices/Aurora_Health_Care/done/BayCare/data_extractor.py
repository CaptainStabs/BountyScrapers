...
insurances = ["Aetna_W","Aetna_PPO","Anthem_Blue_Priority","Anthem_Blue_Preferred","Anthem_PPO","Aurora_Caregiver","Cigna_GPPO","Cigna_PPO","Exchange_-Common_Ground","Exchange_-Molina","Health_EOS_Plus","Health_EOS_PPO","HPS","Humana_WVN","Humana_HMO","Humana_HPN_PPO","Trilogy","UHC_HMO","UHC_PPO","WEA_State_and_Trust","WEA_Broad","WPS_Arise","WPS_Statewide","SELF_PAY","MIN","MAX"]
#                                       ,Chargecode_DRG_CPT  ,Description,    CPT,
columns = ["cms_certification_num", "internal_revenue_code", "description", "code", "price", "inpatient_outpatient", "payer"]
with open("./input_files/insurances.csv", "r", encoding="utf-8") as input_csv:
        ...
        for row in tqdm(reader, total=line_count):
            try:
                if str(row["CPT"]).strip() and str(row["CPT"]) != "NA":
                    ...
                    for i in range(26):
                        price_info["price"] = str(row[insurances[i]]).replace(",", "")
                        price_info["payer"] = str(insurances[i]).upper().replace("_", " ").replace("-", "")
